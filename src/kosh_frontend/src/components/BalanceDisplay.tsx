...
const BalanceDisplay = ({ 
  ...
  const handleRefreshBalance = async () => {
    // Determine which address to check based on network
    ...
    if (!onGetBalance || !currentAddress) {
      toast({
        title: "Cannot refresh balance",
        description: `No ${networkName} address available`,
        variant: "destructive",
      });
      return;
    }
    ...
    try {
      const result = await onGetBalance(currentAddress);
      setBalance(result);
  ...
  useEffect(() => {
    ...
    if (currentAddress && isVisible) {
      handleRefreshBalance();
    }
  }, [stellarAddress?.stellar_address, evmAddress?.evm_address, isVisible, selectedNetwork]);

  // Auto-refresh removed as requested by user

  const formatBalance = (bal: string | null) => {
    if (!bal) return '0.00';
    // Remove both XLM and ETH suffixes to get just the number
    const balanceString = bal.replace(/ XLM| ETH/g, '') || '0';
    ...
    // For XLM (Stellar networks): show only 2 decimal places
    // For ETH (EVM networks): show up to 7 decimal places
    return numBalance.toLocaleString('en-US', {
      minimumFractionDigits: 2,
      maximumFractionDigits: isEvmNetwork ? 7 : 2,
      useGrouping: true
    });
  };
  ...
  const getCurrentCurrency = () => {
    const isEvmNetwork = selectedNetwork === 'holesky-testnet' || selectedNetwork === 'base-mainnet';
    return isEvmNetwork ? 'ETH' : 'XLM';
  };

  const getUsdValue = () => {
    if (!balance || !priceData || !formatUsdValue) return '$0.00';
    // Remove both XLM and ETH suffixes to get just the number
    const numBalance = parseFloat(balance.replace(/ XLM| ETH/g, '') || '0');
    if (isNaN(numBalance)) return '$0.00';
            ...
            <div className="text-3xl sm:text-4xl lg:text-5xl font-bold bg-gradient-to-r from-primary to-crypto-blue bg-clip-text text-transparent break-all text-center max-w-full">
              {balanceLoading ? (
                ...
                <div className="flex items-center justify-center flex-wrap gap-1">
                  ...
                  <span className="text-lg sm:text-xl lg:text-2xl text-crypto-teal">{getCurrentCurrency()}</span>
                </div>
              ) : (
